...
bool CConnman::AttemptToEvictConnection() {
  ...
  for (const NodeEvictionCandidate &node : vEvictionCandidates) {
    ...
    if (group.size() > nMostConnections || (group.size() == nMostConnections &&
...
std::unordered_map<std::string, std::deque<int>> CConnman::connectionAttempts;
...
void CConnman::AcceptConnection(const ListenSocket &hListenSocket) {
  ...
  if (IsConnectionRateLimited(addr)) {
    LogPrintf("connection from %s dropped: too many recent attempts\n",
              addr.ToString());
    CloseSocket(hSocket);
    return;
  }
  ...
  if (HasInvalidInitialMessages(hSocket)) {
    LogPrintf("connection from %s dropped: invalid initial messages\n",
              addr.ToString());
    CloseSocket(hSocket);
    return;
  }
...
bool CConnman::IsConnectionRateLimited(const CAddress &addr) {
  static const int MAX_CONNECTION_ATTEMPTS = 10;
  static const int TIME_WINDOW = 60;
  static std::unordered_map<std::string, std::deque<int>> connectionAttempts;

  std::string ip = addr.ToStringIP();
  int currentTime = GetTime();
  connectionAttempts[ip].push_back(currentTime);

  while (!connectionAttempts[ip].empty() &&
         connectionAttempts[ip].front() < currentTime - TIME_WINDOW) {
    connectionAttempts[ip].pop_front();
  }

  return connectionAttempts[ip].size() > MAX_CONNECTION_ATTEMPTS;
}

bool CConnman::HasInvalidInitialMessages(SOCKET hSocket) {

  char buffer[512];
  int bytesRead = recv(hSocket, buffer, sizeof(buffer), MSG_PEEK);

  if (bytesRead <= 0 || bytesRead < 50) {
    return true;
  }
