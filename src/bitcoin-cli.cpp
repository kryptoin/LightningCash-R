...
std::string HelpMessageCli() {
  ...
  strUsage += HelpMessageOpt(
      ...
      _("Send RPC for non-default wallet on RPC server (argument is wallet "
        "filename in lightningcashrd directory, required if "
        "lightningcashrd/-Qt runs with multiple wallets)"));
  ...
  if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") ||
    ...
    if (!gArgs.IsArgSet("-version")) {
      strUsage +=
          ...
          "  lightningcashr-cli [options] -named <command> [name=value] ... " +
          strprintf(_("Send command to %s (with named arguments)"),
                    _(PACKAGE_NAME)) +
          "\n" + "  lightningcashr-cli [options] help                " +
          _("List commands") + "\n" +
          "  lightningcashr-cli [options] help <command>      " +
          ...
          if (error.isObject()) {
            ...
            if (errCode.isNum() &&
              ...
              strPrint += "\nTry adding \"-rpcwallet=<filename>\" option to "
                          "lightningcashr-cli command line.";
            }
          }
        } else {
          if (result.isNull())
