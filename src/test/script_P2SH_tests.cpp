...
BOOST_AUTO_TEST_CASE(sign)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        BOOST_CHECK_MESSAGE(SignSignature(keystore, txFrom, txTo[i], 0, SIGHASH_ALL | SIGHASH_FORKID), strprintf("SignSignature %d", i)); // LightningCashr Reborn: Replay attack protection
    }
    ...
    for (int i = 0; i < 8; i++) {
        ...
        for (int j = 0; j < 8; j++)
        {
            ...
            bool sigOK = CScriptCheck(output, txTo[i], 0, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_STRICTENC | SCRIPT_ENABLE_SIGHASH_FORKID, false, &txdata)();	// LightningCashr Reborn: Replay attack protection
            if (i == j)
...
BOOST_AUTO_TEST_CASE(AreInputsStandard)
{
    ...
    BOOST_CHECK(SignSignature(keystore, txFrom, txTo, 0, SIGHASH_ALL | SIGHASH_FORKID));	// LightningCashr Reborn: Replay attack protection
    BOOST_CHECK(SignSignature(keystore, txFrom, txTo, 1, SIGHASH_ALL | SIGHASH_FORKID));	// LightningCashr Reborn: Replay attack protection
    BOOST_CHECK(SignSignature(keystore, txFrom, txTo, 2, SIGHASH_ALL | SIGHASH_FORKID));	// LightningCashr Reborn: Replay attack protection
