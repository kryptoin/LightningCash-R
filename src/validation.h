...
#include "chain.h" // For CBlockIndexWorkComparator
#include "pow.h"
...
static const unsigned int DEFAULT_MIN_RELAY_TX_FEE =
    10000 / COIN_SCALE / 10; // LightningCashr: Lowered fees again
//! -maxtxfee default
static const CAmount DEFAULT_TRANSACTION_MAXFEE =
    0.1 * COIN * COIN_SCALE; // LightningCashr: Account for coin scale
//! Discourage users to set fees higher than this amount (in satoshis) per kB
static const CAmount HIGH_TX_FEE_PER_KB =
    0.01 * COIN * COIN_SCALE; // LightningCashr: Account for coin scale
...
static const int SKIP_BLOCKHEADER_POW =
    0; // Skip up to last checkpoint + 1 ( 16832760 )
 ...
 * (which make reindexing and pruning harder). We'll probably want to make this
 * a per-peer adaptive value at some point. */
...
struct CBlockIndexWorkComparator {
  bool operator()(const CBlockIndex *pa, const CBlockIndex *pb) const {
    ...
    if (pa->nSequenceId > pb->nSequenceId)
      return true;
    return pa > pb; // tie-breaker on pointer address
  }
};
...
// and rev???.dat) At 1MB per block, 288 blocks = 288MB. Add 15% for Undo data =
// 331MB Add 20% for Orphan block rate = 397MB We want the low water mark after
// pruning to be at least 397 MB and since we prune in full block file chunks,
// we need the high water mark which triggers the prune to be one 128MB block
// file + added 15% undo data = 147MB greater for a total of 545MB Setting the
// target to > than 550MB will make it likely we can respect the target.
...
public:
  ...
  bool LoadBlockIndex(const Consensus::Params &, CBlockTreeDB &);
  bool ActivateBestChain(CValidationState &, const CChainParams &,
                         std::shared_ptr<const CBlock>);
  bool AcceptBlockHeader(const CBlockHeader &, CValidationState &,
                         const CChainParams &, CBlockIndex **);
  bool AcceptBlock(const std::shared_ptr<const CBlock> &, CValidationState &,
                   const CChainParams &, CBlockIndex **, bool,
                   const CDiskBlockPos *, bool *);
  DisconnectResult DisconnectBlock(const CBlock &, const CBlockIndex *,
                                   CCoinsViewCache &);
  bool ConnectBlock(const CBlock &, CValidationState &, CBlockIndex *,
                    CCoinsViewCache &, const CChainParams &,
                    bool fJustCheck = false);
  bool DisconnectTip(CValidationState &, const CChainParams &,
                     DisconnectedBlockTransactions *);
  bool PreciousBlock(CValidationState &, const CChainParams &, CBlockIndex *);
  bool InvalidateBlock(CValidationState &, const CChainParams &, CBlockIndex *);
  bool ResetBlockFailureFlags(CBlockIndex *);
  bool ReplayBlocks(const CChainParams &, CCoinsView *);
  bool RewindBlockIndex(const CChainParams &);
  bool LoadGenesisBlock(const CChainParams &);
  void PruneBlockIndexCandidates();
  void UnloadBlockIndex();

private:
  bool ActivateBestChainStep(CValidationState &, const CChainParams &,
                             CBlockIndex *,
                             const std::shared_ptr<const CBlock> &, bool &,
                             ConnectTrace &);
  bool ConnectTip(CValidationState &, const CChainParams &, CBlockIndex *,
                  const std::shared_ptr<const CBlock> &, ConnectTrace &,
                  DisconnectedBlockTransactions &);
  CBlockIndex *AddToBlockIndex(const CBlockHeader &);
  CBlockIndex *InsertBlockIndex(const uint256 &);
  void CheckBlockIndex(const Consensus::Params &);
  void InvalidBlockFound(CBlockIndex *, const CValidationState &);
  CBlockIndex *FindMostWorkChain();
  bool ReceivedBlockTransactions(const CBlock &, CValidationState &,
                                 CBlockIndex *, const CDiskBlockPos &,
                                 const Consensus::Params &);
  bool RollforwardBlock(const CBlockIndex *, CCoinsViewCache &,
                        const CChainParams &);
};
 ...
 * wasAlways skip blockheader checking for faster sync time first received via
 * this call
...
CAmount GetBeeCost(
    int nHeight,
    const Consensus::Params &consensusParams); // LightningCashr: Hive: Get the
                                               // cost of a bee at given height
/** Guess verification progress (as a fraction between 0.0=genesis
 * and 1.0=current tip). */
...
// LightningCashr: Hive: Check if Hive 1.2 is activated at given point ( not a
// soft fork, included in the Adjust Fork ! )
bool IsHive12Enabled(int nHeight);

// LightningCashr: Hive: Get the well-rooted deterministic random string (see
// whitepaper section 4.1)
std::string GetDeterministicRandString(const CBlockIndex *pindexPrev);

// LightningCashr: Hive: Get tx by given hash, from a block at given chain
// height
 ...
 * cs_main) This is also true for mempool checks.
 */
