 ...
 * creation) Override with -mintxfee
 */
CFeeRate CWallet::minTxFee = CFeeRate(DEFAULT_TRANSACTION_MINFEE);
/**
 * If fee estimation does not have enough data to provide estimates, use this
 * fee instead. Has no effect if not using fee estimation Override with
 * -fallbackfee
 */
...
bool CWallet::AddToWalletIfInvolvingMe(const CTransactionRef &ptx,
  ...
  const CTransaction &tx = *ptx;
  {
    ...
    if (fExisted || IsMine(tx) || IsFromMe(tx)) {
       ...
       * keypool. This can happen when restoring an old wallet backup that does
       * not contain the mostly recently created transactions from newer
...
bool fWalletUnlockHiveMiningOnly = false;
...
CWallet::GetBCTs(bool includeDead, bool scanRewards,
  ...
  else if ((chainActive.Height() >= nSpeedFork) &&
           (chainActive.Height() < nAdjustFork))
    maxDepth =
        consensusParams.beeGestationBlocks + consensusParams.beeLifespanBlocks3;
  else
    maxDepth =
        consensusParams.beeGestationBlocks + consensusParams.beeLifespanBlocks;
...
std::vector<CBeeCreationTransactionInfo>
CWallet::GetBCTs2(bool includeDead, bool scanRewards,
                  const Consensus::Params &consensusParams,
                  int minHoneyConfirmations) {
  std::vector<CBeeCreationTransactionInfo> bcts;
  ...
  if (chainActive.Height() >= nAdjustFork) {
    ...
    maxDepth2 =
        consensusParams.beeGestationBlocks + consensusParams.beeLifespanBlocks2;
    wititi2 = consensusParams.beeLifespanBlocks2;
  }

  /*    if (chainActive.Height() >= consensusParams.ratioForkBlock) {
          maxDepth = consensusParams.beeGestationBlocks +
         consensusParams.beeLifespanBlocks2; wititi =
         consensusParams.beeLifespanBlocks2;
          }
          else {
          maxDepth = consensusParams.beeGestationBlocks +
         consensusParams.beeLifespanBlocks; wititi =
  ...
  for (const std::pair<uint256, CWalletTx> &pairWtx : mapWallet) {
    ...
    CAmount beeFeePaid;
    CAmount beeCost =
        0.0004 * (GetBlockSubsidy(chainActive.Height(), consensusParams));
    CScript scriptPubKeyHoney;
    if (!wtx.tx->IsBCT(consensusParams, scriptPubKeyBCF, &beeFeePaid,
                       &scriptPubKeyHoney))
      continue;
    ...
    int cocorico = (chainActive.Height() - depth);
    ...
    if (cocorico < consensusParams.ratioForkBlock) {
      ...
      if ((blocksLeft > wititi) && (blocksLeft <= maxDepth)) {
        ...
        if (((torpinouche > totito) &&
             ((ciboire > totito) && (ciboire < torpinouche))) ||
            ((totito > torpinouche) &&
             ((ciboire > totito) || (ciboire < torpinouche)))) {
      ...
      if (blocksLeft < 1) {
        ...
        if (((torpinouche > totito) &&
             ((ciboire3 > totito) && (ciboire3 < torpinouche))) ||
            ((totito > torpinouche) &&
             ((ciboire3 > totito) || (ciboire3 < torpinouche)))) {
      ...
      if ((blocksLeft <= wititi) && (blocksLeft >= 1)) {
        ...
        if (((torpinouche > totito) &&
             ((ciboire2 > totito) && (ciboire2 < torpinouche))) ||
            ((totito > torpinouche) &&
             ((ciboire2 > totito) || (ciboire2 < torpinouche)))) {
    ...
    if (cocorico >= consensusParams.ratioForkBlock) {
      ...
      int blocksLeft = maxDepth2 - depth;
...
std::vector<CBeeCreationTransactionInfo>
CWallet::GetBCTs3(bool includeDead, bool scanRewards,
                  const Consensus::Params &consensusParams,
                  int minHoneyConfirmations) {
  std::vector<CBeeCreationTransactionInfo> bcts;
...
bool CWallet::CreateBeeTransaction(
  ...
  if (chainActive.Height() >= nContribFork)
    donationValue =
        (CAmount)(totalBeeCost / consensusParams.communityContribFactor2);
  else
    donationValue =
...
bool CWallet::CreateBeeTransaction2(
    int beeCount, CWalletTx &wtxNew, CReserveKey &reservekeyChange,
    CReserveKey &reservekeyHoney, std::string honeyAddress,
    std::string changeAddress, bool communityContrib,
    std::string &strFailReason, const Consensus::Params &consensusParams) {
...
bool CWallet::CreateBeeTransaction3(
    int beeCount, CWalletTx &wtxNew, CReserveKey &reservekeyChange,
    CReserveKey &reservekeyHoney, std::string honeyAddress,
    std::string changeAddress, bool communityContrib,
    std::string &strFailReason, const Consensus::Params &consensusParams) {
...
bool CWallet::CreateTransaction(const std::vector<CRecipient> &vecSend,
  ...
  try {
    ...
    strFailReason = std::string("Transaction creation failed: ") + e.what();
    LogPrintf("%s\n", strFailReason);
    return false;
  } catch (...) {
    strFailReason = "Transaction creation failed: unknown exception";
    LogPrintf("%s\n", strFailReason);
    return false;
  }
}
...
// 		ret = reservekey.GetReservedKey(vchPubKey, true);
// 		if (!ret) {
// 		  strFailReason = _("Keypool ran out, please call keypoolrefill
// first"); 		  return false;
// 		}

// 		const OutputType change_type =
// 			TransactionChangeType(coin_control.change_type,
// vecSend);
...
// "after the fee has been deducted"); 			} else
// strFailReason =
// _("Transaction amount too small"); 			return false;
// 		  }
...
// 		  if (!SelectCoins(vAvailableCoins, nValueToSelect, setCoins,
// nValueIn, 						   &coin_control)) {
// strFailReason = _("Insufficient funds"); 			return false;
// 		  }
// 		}
...
// txNew.vout.size()) { 			  strFailReason = _("Change
...
// 									   ?
// MAX_BIP125_RBF_SEQUENCE
// : (CTxIn::SEQUENCE_FINAL - 1); 		for (const auto &coin :
// setCoins) 		  txNew.vin.push_back(CTxIn(coin.outpoint, CScript(),
// nSequence));
...
// 				nBytes + change_prototype_size + 2;
// 			CAmount fee_needed_with_change =
// 				GetMinimumFee(tx_size_with_change, coin_control,
// ::mempool,
// 							  ::feeEstimator,
// nullptr); 			CAmount minimum_value_for_change =
// 				GetDustThreshold(change_prototype_txout,
// discard_rate); 			if (nFeeRet >= fee_needed_with_change +
// minimum_value_for_change) { 			  pick_new_inputs = false;
// nFeeRet = fee_needed_with_change; 			  continue;
// 			}
// 		  }
...
// 		  strFailReason = _("Transaction fee and change calculation
// failed"); 		  return false;
// 		}
...
// 		if (!ProduceSignature(
// 				TransactionSignatureCreator(this, &txNewConst,
// nIn,
// coin.txout.nValue, SIGHASH_ALL), 				scriptPubKey,
// sigdata)) { 		  strFailReason = _("Signing transaction failed");
// return false; 		} else { 		  UpdateTransaction(txNew, nIn, sigdata);
// 		}
...
// 	size_t nLimitAncestorSize =
// 		gArgs.GetArg("-limitancestorsize", DEFAULT_ANCESTOR_SIZE_LIMIT)
// * 1000; 	size_t nLimitDescendants =
// gArgs.GetArg("-limitdescendantcount", DEFAULT_DESCENDANT_LIMIT); 	size_t
// nLimitDescendantSize = 		gArgs.GetArg("-limitdescendantsize",
// DEFAULT_DESCENDANT_SIZE_LIMIT) * 		1000; 	std::string errString;
// if
// (!mempool.CalculateMemPoolAncestors( 			entry,
// setAncestors, nLimitAncestors, nLimitAncestorSize,
// nLimitDescendants, nLimitDescendantSize, errString)) { 	  strFailReason =
// _("Transaction has too long of a mempool chain"); 	  return false;
// 	}
//   }
...
// feeCalc.est.pass.withinTarget, feeCalc.est.pass.totalConfirmed,
// feeCalc.est.pass.inMempool, feeCalc.est.pass.leftMempool,
// feeCalc.est.fail.start, feeCalc.est.fail.end, 	  100 *
// feeCalc.est.fail.withinTarget / 		  (feeCalc.est.fail.totalConfirmed +
...
bool CWalletTx::AcceptToMemoryPool(const CAmount &nAbsurdFee,
  ...
  bool ret = ::AcceptToMemoryPool(
      mempool, state, tx, nullptr /* pfMissingInputs */,
