...
extern bool fWalletUnlockHiveMiningOnly; // LightningCashr: Hive: Unlock for
                                         // hive mining purposes only.
...
static const CAmount DEFAULT_FALLBACK_FEE =
    2000000 / COIN_SCALE /
    10; // LightningCashr: 10X cheaper fees. Note these constants must also
        // account for the 10X coinscale.
...
enum WalletFeature {
  FEATURE_BASE = 10500, // the earliest version new wallets supports (only
                        // useful for getwalletinfo's clientversion output)
...
public:
  ...
  int GetDepthInOrder() const {
    const CBlockIndex *pindexRet;
    return GetDepthInOrder(pindexRet);
  }
  ...
  bool IsHiveCoinBase() const {
    return tx->IsHiveCoinBase();
  } // LightningCashr: Hive

  // LightningCashr: Hive: Check if this transaction is a Bee Creation
  // Transaction
...
class WalletRescanReserver; // forward declarations for
                            // ScanForWalletTransactions/RescanFromTime
/**
...
public:
  ...
  bool SignHiveProof(const CBeeCreationTransactionInfo &solvingBee,
                     const std::string &deterministicRandString,
                     std::vector<unsigned char> &vchSigOut) const;
  ...
  bool CreateBeeTransaction2(int beeCount, CWalletTx &wtxNew,
                             CReserveKey &reservekeyChange,
                             CReserveKey &reservekeyHoney,
                             std::string honeyAddress,
  ...
  bool CreateBeeTransaction3(int beeCount, CWalletTx &wtxNew,
                             CReserveKey &reservekeyChange,
                             CReserveKey &reservekeyHoney,
                             std::string honeyAddress,
                             std::string changeAddress, bool communityContrib,
  ...
  // LightningCashr: Hive: Return all BCTs known by this wallet, optionally
  // including dead bees and optionally scanning for blocks minted by bees from
  // each BCT
  ...
  std::vector<CBeeCreationTransactionInfo>
  GetBCTs2(bool includeDead, bool scanRewards,
           const Consensus::Params &consensusParams,
           int minHoneyConfirmations = 1);
  std::vector<CBeeCreationTransactionInfo>
  GetBCTs3(bool includeDead, bool scanRewards,
           const Consensus::Params &consensusParams,
           int minHoneyConfirmations = 1);
...
/** Get all destinations (potentially) supported by the wallet for the given
 * key. */
