  ...
  bool fRescan = true;
  {
    ...
    if (fWalletUnlockHiveMiningOnly)
      ...
      throw JSONRPCError(RPC_WALLET_UNLOCK_NEEDED,
                         "Wallet is unlocked for hive mining only.");
...
UniValue importaddress(const JSONRPCRequest &request) {
...

  {
    ...
    if (IsValidDestination(dest)) {
      ...
      throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                         "Invalid LightningCashr address or script");
    }
  }
  if (fRescan) {
    pwallet->RescanFromTime(TIMESTAMP_MIN, reserver, true);
...
UniValue dumpprivkey(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"address\"   (string, required) The lightningcashr address for "
        "the private key\n"
        "\nResult:\n"
...
UniValue dumpwallet(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "absolute or relative to lightningcashrd)\n"
        "\nResult:\n"
        "{                           (json object)\n"
          ...
          result.pushKV(
              ...
              JSONRPCError(
                  ...
                  strprintf("Rescan failed for key with creation timestamp %d. "
                            ...
                            "lightningcashrd log for details) and could "
