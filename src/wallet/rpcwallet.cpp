...
void EnsureWalletIsUnlocked(CWallet *const pwallet) {
  ...
  if (fWalletUnlockHiveMiningOnly) {
    throw JSONRPCError(
        ...
        "Error: Wallet is unlocked for hive mining only, please enter the "
...
UniValue getaccountaddress(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "\nDEPRECATED. Returns the current LightningCashr address for "
...
UniValue setaccount(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"address\"         (string, required) The lightningcashr address "
  ...
  if (!IsValidDestination(dest)) {
    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                       "Invalid LightningCashr address");
  }
...
UniValue getaddressesbyaccount(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "  \"address\"         (string) a lightningcashr address associated "
...
UniValue sendtoaddress(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"address\"            (string, required) The lightningcashr "
        "address to send to.\n"
        ...
        "lightningcashr than you enter in the amount field.\n"
...
UniValue createbees(const JSONRPCRequest &request) {
  ...
  if ((chainActive.Tip()->nHeight) >= nSpeedFork) {
    if (pwallet->CreateBeeTransaction(beeCount, wtxNew, reservekeyChange,
                                      reservekeyHoney, honeyAddress,
                                      changeAddress, communityContrib, strError,
                                      Params().GetConsensus())) {
      CValidationState state;
  ...
  if ((Params().GetConsensus().variableBeecost) &&
      (((chainActive.Tip()->nHeight) - 1) >=
       (Params().GetConsensus().variableForkBlock)) &&
      (((chainActive.Tip()->nHeight) - 1) >=
       (Params().GetConsensus().remvariableForkBlock)) &&
      ((chainActive.Tip()->nHeight) < nSpeedFork)) {
    if (pwallet->CreateBeeTransaction3(beeCount, wtxNew, reservekeyChange,
                                       reservekeyHoney, honeyAddress,
                                       changeAddress, communityContrib,
                                       strError, Params().GetConsensus())) {
      CValidationState state;
      if (honeyAddress.empty())
  ...
  if ((Params().GetConsensus().variableBeecost) &&
      ...
      (((chainActive.Tip()->nHeight) - 1) <
       (Params().GetConsensus().remvariableForkBlock)) &&
      ((chainActive.Tip()->nHeight) < nSpeedFork)) {
    if (pwallet->CreateBeeTransaction2(beeCount, wtxNew, reservekeyChange,
                                       reservekeyHoney, honeyAddress,
                                       changeAddress, communityContrib,
                                       strError, Params().GetConsensus())) {
      CValidationState state;
...
UniValue getnetworkhiveinfo(const JSONRPCRequest &request) {
  ...
  if ((chainActive.Tip()->nHeight) >= nSpeedFork) {
    if (!GetNetworkHiveInfo4(globalImmatureBees, globalImmatureBCTs,
                             globalMatureBees, globalMatureBCTs,
                             potentialRewards, consensusParams, includeGraph))
      throw std::runtime_error(
  ...
  if ((consensusParams.variableBeecost) &&
      ...
      (((chainActive.Tip()->nHeight) - 1) >=
       (consensusParams.ratioForkBlock)) &&
      (((chainActive.Tip()->nHeight) - 1) >=
  ...
  if ((consensusParams.variableBeecost) &&
      ...
      (((chainActive.Tip()->nHeight) - 1) <
       (consensusParams.remvariableForkBlock)) &&
      ((chainActive.Tip()->nHeight) < nSpeedFork)) {
    if (!GetNetworkHiveInfo3(globalImmatureBees, globalImmatureBCTs,
                             globalMatureBees, globalMatureBCTs,
                             potentialRewards, consensusParams, includeGraph))
      throw std::runtime_error(
          "Error: A block required to calculate network bee population was not "
  ...
  if ((consensusParams.variableBeecost) &&
      (((chainActive.Tip()->nHeight) - 1) >=
       (consensusParams.variableForkBlock)) &&
      (((chainActive.Tip()->nHeight) - 1) < (consensusParams.ratioForkBlock)) &&
      ((chainActive.Tip()->nHeight) < nSpeedFork)) {
    if (!GetNetworkHiveInfo2(globalImmatureBees, globalImmatureBCTs,
                             globalMatureBees, globalMatureBCTs,
                             potentialRewards, consensusParams, includeGraph))
      throw std::runtime_error(
          "Error: A block required to calculate network bee population was not "
  ...
  if (includeGraph) {
    ...
    if (((chainActive.Tip()->nHeight) - 1) >= nAdjustFork)
      totalBeeLifespan = consensusParams.beeLifespanBlocks +
                         consensusParams.beeGestationBlocks;
    if ((((chainActive.Tip()->nHeight) - 1) >= nSpeedFork) &&
        (((chainActive.Tip()->nHeight) - 1) < nAdjustFork))
      totalBeeLifespan = consensusParams.beeLifespanBlocks3 +
                         consensusParams.beeGestationBlocks;
    if ((((chainActive.Tip()->nHeight) - 1) >=
         (consensusParams.ratioForkBlock)) &&
        (((chainActive.Tip()->nHeight) - 1) < nSpeedFork))
      totalBeeLifespan = consensusParams.beeLifespanBlocks2 +
                         consensusParams.beeGestationBlocks;
...
UniValue gethiveinfo(const JSONRPCRequest &request) {
  ...
  if ((consensusParams.variableBeecost) &&
      ...
      ((chainActive.Tip()->nHeight) < nSpeedFork)) {
    bcts =
        pwallet->GetBCTs3(includeDead, true, consensusParams, minHoneyConfirms);
  }
  if ((consensusParams.variableBeecost) &&
      (((chainActive.Tip()->nHeight) - 1) >=
       (consensusParams.variableForkBlock)) &&
      ...
      ((chainActive.Tip()->nHeight) < nSpeedFork)) {
    bcts =
        pwallet->GetBCTs2(includeDead, true, consensusParams, minHoneyConfirms);
  }
  if ((consensusParams.variableBeecost) &&
      (((chainActive.Tip()->nHeight) - 1) <
       (consensusParams.variableForkBlock)) &&
...
UniValue sendfrom(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 3 || request.params.size() > 6)
    throw std::runtime_error(
        ...
        "                       the spend.\n"
        "2. \"toaddress\"         (string, required) The lightningcashr "
...
UniValue sendmany(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
    throw std::runtime_error(
        ...
        "    {\n"
        "      \"address\":amount   (numeric or string) The lightningcashr "
        ...
        "lightningcashr than you enter in their corresponding amount field.\n"
  ...
  for (const std::string &name_ : keys) {
    ...
    if (!IsValidDestination(dest)) {
      throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                         std::string("Invalid LightningCashr address: ") +
                             name_);
    }
...
UniValue addmultisigaddress(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 4) {
    std::string msg =
        ...
        "of lightningcashr addresses or hex-encoded public keys\n"
        "     [\n"
        "       \"address\"                  (string) lightningcashr address "
        "or hex-encoded public key\n"
        "       ...,\n"
        "     ]\n"
        ...
        "start lightningcashrd with -deprecatedrpc=addmultisigaddress).\n"
        "        clients should transition to the new output api before "
        "upgrading to v0.17.\n"
        "\"address\"                         (string) A lightningcashr address "
...
UniValue addwitnessaddress(const JSONRPCRequest &request) {
  ...
  if (!IsDeprecatedRPCEnabled("addwitnessaddress")) {
    throw JSONRPCError(
        ...
        "To use addwitnessaddress in v0.16, restart lightningcashrd with "
...
UniValue gettransaction(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "account.\n"
        "      \"address\" : \"address\",          (string) The lightningcashr "
...
static void SetHiveOnly() { fWalletUnlockHiveMiningOnly = true; }
...
UniValue walletpassphrase(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 2) {
    throw std::runtime_error(
        ...
        "keys such as sending lightningcashr\n"
        "\nArguments:\n"
...
UniValue walletpassphrasehiveonly(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1) {
    throw std::runtime_error(
        ...
        "mining use only.\n"
        "This is needed to enable the hive mining thread to run. Performing "
        "other transactions related to\n"
        "private keys such as sending lightningcashr, is not enabled and will "
        "require you to run\n"
  ...
  if (!pwallet->IsCrypted()) {
    throw JSONRPCError(RPC_WALLET_WRONG_ENC_STATE,
                       "Error: running with an unencrypted wallet, but "
                       "walletpassphrasehiveonly was called.");
  }
  ...
  if (strWalletPass.length() > 0) {
    ...
    throw std::runtime_error("walletpassphrasehiveonly <passphrase>\n"
                             "Stores the wallet decryption key in memory "
                             "indefinitely for hive mining use only.");
...
UniValue encryptwallet(const JSONRPCRequest &request) {
  ...
  StartShutdown();
  return "wallet encrypted; LightningCashr server stopping, restart to run "
...
UniValue lockunspent(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "selection, when spending lightningcashr.\n"
...
UniValue listunspent(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() > 5)
    throw std::runtime_error(
        ...
        "confirmations to filter\n"
        "3. \"addresses\"      (string) A json array of lightningcashr "
        "addresses to filter\n"
        "    [\n"
        "      \"address\"     (string) lightningcashr address\n"
        "      ,...\n"
...
UniValue fundrawtransaction(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
    throw std::runtime_error(
        ...
        "address) The lightningcashr address to receive the change\n"
    ...
    if (request.params[1].type() == UniValue::VBOOL) {
      ...
      if (options.exists("changeAddress")) {
        ...
        if (!IsValidDestination(dest)) {
          throw JSONRPCError(
              RPC_INVALID_ADDRESS_OR_KEY,
              "changeAddress must be a valid lightningcashr address");
        }
