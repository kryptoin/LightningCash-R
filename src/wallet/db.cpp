...
bool CDBEnv::Open(const fs::path &pathIn, bool retry) {
  ...
  if (!LockDirectory(pathIn, ".walletlock")) {
    LogPrintf("Cannot obtain a lock on wallet directory %s. Another instance "
              "of lightningcashr may be using it.\n",
              strPath);
    return false;
  }
...
bool CopyFile(const std::string &pathSrc, const std::string &pathDest) {
  std::ifstream src(pathSrc, std::ios::binary);
  ...
  while (true) {
    {
      ...
      if (!env->mapFileUseCount.count(strFile) ||
        ...
        try {
          ...
          if (CopyFile(pathSrc.string(), pathDest.string())) {
            LogPrintf("copied %s to %s\n", strFile, pathDest);
            return true;
          } else {
            LogPrintf("error copying %s to %s\n", strFile, pathDest);
            return false;
          }
