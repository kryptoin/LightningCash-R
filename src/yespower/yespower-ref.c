...
#warning                                                                       \
    "This reference implementation is deliberately mostly not optimized. Use yespower-opt.c instead unless you're testing (against) the reference implementation on purpose."
...
static void smix1(uint32_t *B, size_t r, uint32_t N, uint32_t *V, uint32_t *X,
                  pwxform_ctx_t *ctx) {
  size_t s = 32 * r;
  uint32_t i, j;
  size_t k;
...
static void smix(uint32_t *B, size_t r, uint32_t N, uint32_t *V, uint32_t *X,
                 pwxform_ctx_t *ctx) {
...
int yespower(yespower_local_t *local, const uint8_t *src, size_t srclen,
  ...
  if ((version != YESPOWER_0_5 && version != YESPOWER_1_0) || N < 1024 ||
      N > 512 * 1024 || r < 8 || r > 32 || (N & (N - 1)) != 0 || r < rmin ||
      (!pers && perslen)) {
    errno = EINVAL;
    return -1;
  }
  ...
  PBKDF2_SHA256((uint8_t *)sha256, sizeof(sha256), src, srclen, 1, (uint8_t *)B,
                B_size);
  ...
  if (version == YESPOWER_0_5) {
    PBKDF2_SHA256((uint8_t *)sha256, sizeof(sha256), (uint8_t *)B, B_size, 1,
    ...
    HMAC_SHA256_Buf((uint8_t *)B + B_size - 64, 64, sha256, sizeof(sha256),
                    (uint8_t *)dst);
  }
