...
static void print_PBKDF2_SHA256_raw(const char *passwd, size_t passwdlen,
                                    ...
                                    uint64_t c, size_t dkLen) {
  uint8_t dk[64];
  size_t i;

  assert(dkLen <= sizeof(dk));

  printf("PBKDF2_SHA256(\"%s\", \"%s\", %llu, %llu) = ", passwd, salt,
         (unsigned long long)c, (unsigned long long)dkLen);
...
static void print_PBKDF2_SHA256(const char *passwd, const char *salt,
                                uint64_t c, size_t dkLen) {
  print_PBKDF2_SHA256_raw(passwd, strlen(passwd), salt, strlen(salt), c, dkLen);
}
#endif
...
static void print_yespower(yespower_version_t version, uint32_t N, uint32_t r,
                           const char *pers) {
  yespower_params_t params = {.version = version,
                              .N = N,
                              .r = r,
  ...
  printf("yespower(%u, %u, %u, %s%s%s) = ", (unsigned int)version, N, r, q,
         pers ? pers : "NULL", q);
