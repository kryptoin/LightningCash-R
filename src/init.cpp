...
std::string LicenseInfo() {
  const std::string URL_SOURCE_CODE =
      "<https://github.com/diabaths/LightningCashr-Coin>";
...
bool AppInitMain() {
  ...
  if (gArgs.IsArgSet("-datadir") &&
    ...
    LogPrintf("Warning: relative datadir option '%s' specified, which will be "
              ...
              "lightningcashr is started in the future "
              "from a different location, it will be unable to locate the "
              "current data files. There could "
              "also be data loss if lightningcashr is started while in a "
              "temporary directory.\n",
  ...
  peerLogic.reset(
      new PeerLogicValidation(&connman, scheduler, Params().GetConsensus()));
...
#ifdef ENABLE_WALLET
  threadGroup.create_thread([&chainparams] {
    try {
      BeeKeeper(chainparams);
    } catch (const std::exception &e) {
      LogPrintf("Exception in BeeKeeper thread: %s\n", e.what());
    } catch (...) {
      LogPrintf("Unknown exception in BeeKeeper thread\n");
    }
  });
#endif
