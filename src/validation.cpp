...
const int nYesPowerFork = 10;
const int nSpeedFork = 10;
const int nAdjustFork = 20;
const int nLightFork = 20;
const int nHiveRepairFork = 20;
const int nContribFork = 3864378;
...
void UpdateMempoolForReorg(DisconnectedBlockTransactions &disconnectpool,
  ...
  while (it != disconnectpool.queuedTx.get<insertion_order>().rend()) {
    ...
    if (!fAddToMempool || (*it)->IsCoinBase()) {
      ...
      fAdd = AcceptToMemoryPool(mempool, stateDummy, *it,
  ...
  if (chainActive.Height() - chainActive.Tip()->nHeight <= 3) {
...

  {
    ...
    for (const CTxMemPool::txiter it : allConflicting) {
      LogPrint(BCLog::MEMPOOL,
               "replacing tx %s with %s for %s LNCR additional fees, %d delta "
               "bytes\n",
...
CAmount GetBeeCost(int nHeight, const Consensus::Params &consensusParams) {
  ...
  if (chainActive.Height() >= nAdjustFork)
    potentialLifespanRewards = (consensusParams.beeLifespanBlocks *
                                GetBlockSubsidy(nHeight, consensusParams)) /
                               consensusParams.hiveBlockSpacingTarget;

  if ((chainActive.Height() >= nSpeedFork) &&
      (chainActive.Height() < nAdjustFork))
    potentialLifespanRewards = (consensusParams.beeLifespanBlocks3 *
                                GetBlockSubsidy(nHeight, consensusParams)) /
                               consensusParams.hiveBlockSpacingTarget;

  if ((chainActive.Height() >= consensusParams.ratioForkBlock) &&
      (chainActive.Height() < nSpeedFork))
    potentialLifespanRewards = (consensusParams.beeLifespanBlocks2 *
                                GetBlockSubsidy(nHeight, consensusParams)) /
                               consensusParams.hiveBlockSpacingTarget;
  if (chainActive.Height() < consensusParams.ratioForkBlock)
    potentialLifespanRewards = (consensusParams.beeLifespanBlocks *
  ...
  CAmount voyon = 10000000 / ((GetBlockSubsidy(nHeight, consensusParams) /
                               consensusParams.beeCostFactor));
  ...
  if (totalMatureBees > 0) {
    if (totalMatureBees > (haha * 0.9)) {
      ...
      return adjustedBeeCost <= consensusParams.minBeeCost
                 ? consensusParams.minBeeCost
                 : adjustedBeeCost;
    } else {
 ...
 * the corresponding cache which are matched. This is useful for checking blocks
 ...
 */
DisconnectResult ApplyTxInUndo(Coin &&undo, CCoinsViewCache &view,
                               const COutPoint &out) {
  bool fClean = true;
  ...
  return fClean ? DisconnectResult::DISCONNECT_OK
                : DisconnectResult::DISCONNECT_UNCLEAN;
}

/** Undo the effects of this block (with given index) on the UTXO set
 * represented by coins. When FAILED is returned, view is left in an
 * indeterminate state. */
...
bool CChainState::ConnectBlock(const CBlock &block, CValidationState &state,
  ...
  for (unsigned int i = 0; i < block.vtx.size(); i++) {
    ...
    if (!tx.IsCoinBase()) {
      ...
      bool fCacheResults =
          fJustCheck; /* Don't cache results if we're actually connecting
 ...
 * with cs_main held.
 *
 * If disconnectpool is nullptr, then no disconnected transactions are added
 * to disconnectpool (note that the caller is responsible for mempool
...
bool CChainState::DisconnectTip(CValidationState &state,
  ...
  int64_t nStart = GetTimeMicros();
  {
    ...
    if (this->DisconnectBlock(block, pindexDelete, view) !=
        DisconnectResult::DISCONNECT_OK)
 ...
 *
 * This class assumes (and asserts) that the conflicted transactions for a
 * given block are added via mempool callbacks prior to the BlockConnected()
...
/**
 * Connect a new block to chainActive. pblock is either nullptr or a pointer
 * to a CBlock corresponding to pindexNew, to bypass loading it again from
 * disk.
 *
 ...
 * or an activated best chain. pblock is either nullptr or a pointer to a
 * block that is already loaded (to avoid loading it again from disk).
 */
...
static bool CheckBlockHeader(const CBlockHeader &block, CValidationState &state,
  ...
  if (nHeight >= nSpeedFork) {
    ...
    if (fCheckPOW && !block.IsHiveMined(consensusParams)) {
      if (!CheckProofOfWork2(IsYesPower(nHeight) ? block.GetHashYespower()
                                                 : block.GetPoWHash(),
    ...
    if (fCheckPOW && !block.IsHiveMined(consensusParams)) {
      if (!CheckProofOfWork(IsYesPower(nHeight) ? block.GetHashYespower()
                                                : block.GetPoWHash(),
                            block.nBits, consensusParams))
        return state.DoS(50, false, REJECT_INVALID, "high-hash", false,
                         "proof of work failed");
...
bool IsHive11Enabled(const CBlockIndex *pindexPrev,
  ...
  return ((VersionBitsState(pindexPrev, params, Consensus::DEPLOYMENT_HIVE_1_1,
                            versionbitscache) == THRESHOLD_ACTIVE) &&
          (!IsHive12Enabled(pindexPrev->nHeight)));
}

bool IsHive12Enabled(int nHeight) { return (nHeight >= nAdjustFork); }
...
arith_uint256 CalculateChainWork(uint32_t nBits,
                                 const arith_uint256 &previousChainWork,
                                 bool fIsHive = false) {
  if (fIsHive) {
    arith_uint256 hiveWork = arith_uint256(1) << 50;
    return previousChainWork + hiveWork;
  }
  ...
  if (fNegative || target == 0 || fOverflow)
    return 0;

  arith_uint256 blockWork = (~arith_uint256(0) / (target + 1));
  return previousChainWork + blockWork;
}

bool IsHiveMinedHeader(const CBlockHeader &header,
                       const Consensus::Params &params) {
...
static bool ContextualCheckBlockHeader(const CBlockHeader &block,
  ...
  CBlockIndex blockIndex;
  blockIndex.pprev = const_cast<CBlockIndex *>(pindexPrev);
  blockIndex.nHeight = nHeight;
  blockIndex.nChainWork =
      CalculateChainWork(block.nBits, pindexPrev->nChainWork);
  ...
  if (nHeight <= pindexPrev->nHeight) {
    ...
    if (reorgDepth <= 3) {
      ...
      if (blockIndex.nChainWork < pindexPrev->nChainWork * 1.1) {
        return state.DoS(100, false, REJECT_INVALID, "insufficient-work", false,
                         "block has insufficient work for a shallow reorg");
      }
      if (block.GetBlockTime() - pindexPrev->GetBlockTime() < 60) {
        return state.DoS(100, false, REJECT_INVALID, "rapid-reorg", false,
...
/**
 * Contextual checks for blocks, focusing on transaction validation and
 * consensus rules. Validates components inside the block such as
 * transactions, witness data, and weight.
 */
static bool ContextualCheckBlock(const CBlock &block, CValidationState &state,
  ...
  if (VersionBitsState(pindexPrev, consensusParams,
    ...
    if (commitpos != -1) {
      ...
      if (block.vtx[0]->vin[0].scriptWitness.stack.size() != 1 ||
        ...
        return state.DoS(100, false, REJECT_INVALID, "bad-witness-nonce-size",
                         true,
  ...
  if (pindexPrev) {
    ...
    blockIndex.nChainWork =
        CalculateChainWork(block.nBits, pindexPrev->nChainWork,
                           block.IsHiveMined(consensusParams));
    ...
    if (nHeight <= pindexPrev->nHeight) {
      ...
      if (reorgDepth <= 3) {
        ...
        if (blockIndex.nChainWork < pindexPrev->nChainWork * 1.1) {
          return state.DoS(100, false, REJECT_INVALID, "insufficient-work",
                           false,
                           "block has insufficient work for a shallow reorg");
        }
...
/**
 * Store a block header in the block index and validate it against consensus
 * rules. This is part of the initial block header synchronization and
 * validation process.
 */
bool CChainState::AcceptBlockHeader(const CBlockHeader &block,
  ...
  if (hash != chainparams.GetConsensus().hashGenesisBlock) {
    ...
    tempIndex.pprev = const_cast<CBlockIndex *>(pindexPrev);
    tempIndex.nHeight = nHeight;
    tempIndex.nChainWork = CalculateChainWork(
...
/**
 * Store a block on disk and validate it against consensus rules.
 * This extends header validation by checking the full block content
 * and handling the block's transactions and chainstate updates.
 */
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock> &pblock,
  ...
  if (!fRequested) {
    ...
    if (pindex->nTx != 0 || !fHasMoreOrSameWork || fTooFarAhead ||
        pindex->nChainWork < nMinimumChainWork) {
      return true;
    }
  }
  ...
  try {
    ...
    if (blockPos.IsNull()) {
      state.Error(strprintf("%s: Failed to find position to write new block "
                            "to disk at height %d",
    ...
    if (!ReceivedBlockTransactions(block, state, pindex, blockPos,
      ...
      return error("%s: ReceivedBlockTransactions failed for block %s",
                   __func__, block.GetHash().ToString());
    }
  } catch (const std::runtime_error &e) {
...
/** Apply the effects of a block on the utxo cache, ignoring that it may
...
bool CChainState::LoadGenesisBlock(const CChainParams &chainparams) {
  ...
  try {
    ...
    CDiskBlockPos blockPos =
        SaveBlockToDisk(block, 0, chainparams, (const CDiskBlockPos *)nullptr);
    if (blockPos.IsNull())
    ...
    while (num--) {
      ...
      if (nTime + nExpiryTimeout > nNow) {
        ...
        AcceptToMemoryPoolWithTime(
            chainparams, mempool, state, tx, nullptr /* pfMissingInputs */,
            nTime, nullptr /* plTxnReplaced */, false /* bypass_limits */,
            0 /* nAbsurdFee */);
        if (state.IsValid()) {
          ++count;
        } else {
...
bool IsYesPower(int nHeight) { return (nHeight >= nYesPowerFork); }

bool IsSpeedFork(int nHeight) { return (nHeight >= nSpeedFork); }
