...
void HiveDialog::updateData2(bool forceGlobalSummaryUpdate) {
  ...
  if (forceGlobalSummaryUpdate ||
    ...
    if ((consensusParams.variableBeecost) &&
      ...
      if (!GetNetworkHiveInfo4(globalImmatureBees, globalImmatureBCTs,
        ...
        if (flute == 0)
          ...
          ui->globalMatureLabel->setText(formatLargeNoLocale(flute) + " (" +
                                         QString::number(globalMatureBCTs) +
                                         " transactions)");
    ...
    CAmount npotentialRewards =
        (consensusParams.beeLifespanBlocks *
         GetBlockSubsidy(chainActive.Tip()->nHeight, consensusParams)) /
        consensusParams.hiveBlockSpacingTarget;

    beePopIndex =
        (((0.0004 * (GetBlockSubsidy(HeightX, consensusParams))) * flute) /
         (double)npotentialRewards) *
        100.0;
...
void HiveDialog::on_createBeesButton_clicked() {
  ...
  if (model) {
    ...
    model->createBees(ui->beeCountSpinner->value(),
                      ui->donateCommunityFundCheckbox->isChecked(), this,
                      beePopIndex);
  }
}
...
void HiveDialog::updateGraph() {
  ...
  int beeCostStable =
      0.0004 * (GetBlockSubsidy(chainActive.Tip()->nHeight, consensusParams));
  ...
  globalMarkerLine->end->setCoords(
      now + consensusParams.nPowTargetSpacing2 / 2 * totalLifespan, global100);
