...
WalletModel::UnlockContext WalletModel::requestUnlock(bool hiveOnly) {
  ...
  return UnlockContext(this, valid, was_locked && !fWalletUnlockHiveMiningOnly);
}
...
void WalletModel::getBCTs(
  ...
  if (wallet) {
    ...
    if ((Params().GetConsensus().variableBeecost) &&
      ...
      vBeeCreationTransactions =
          wallet->GetBCTs3(includeDeadBees, true, Params().GetConsensus());
    }
    if ((Params().GetConsensus().variableBeecost) &&
        (((chainActive.Tip()->nHeight) - 1) >=
         (Params().GetConsensus().variableForkBlock)) &&
        ...
        ((chainActive.Tip()->nHeight) < nSpeedFork)) {
      vBeeCreationTransactions =
          wallet->GetBCTs2(includeDeadBees, true, Params().GetConsensus());
    }
    if ((Params().GetConsensus().variableBeecost) &&
        (((chainActive.Tip()->nHeight) - 1) <
         (Params().GetConsensus().variableForkBlock)) &&
...
bool WalletModel::createBees(int beeCount, bool communityContrib,
  ...
  if ((chainActive.Tip()->nHeight) >= nSpeedFork) {
    if (!wallet->CreateBeeTransaction(beeCount, wtxNew, reservekeyChange,
  ...
  if ((Params().GetConsensus().variableBeecost) &&
      ...
      (((chainActive.Tip()->nHeight) - 1) >=
       (Params().GetConsensus().remvariableForkBlock)) &&
      ((chainActive.Tip()->nHeight) < nSpeedFork)) {
    if (!wallet->CreateBeeTransaction3(beeCount, wtxNew, reservekeyChange,
                                       reservekeyHoney, honeyAddress,
  ...
  if ((Params().GetConsensus().variableBeecost) &&
      ...
      (((chainActive.Tip()->nHeight) - 1) <
       (Params().GetConsensus().remvariableForkBlock)) &&
      ((chainActive.Tip()->nHeight) < nSpeedFork)) {
    if (!wallet->CreateBeeTransaction2(beeCount, wtxNew, reservekeyChange,
                                       reservekeyHoney, honeyAddress,
                                       changeAddress, communityContrib,
                                       strError, Params().GetConsensus())) {
      QMessageBox::critical(parent, tr("Error"),
