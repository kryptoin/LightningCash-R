...
// Optimized merkle computation with better memory management
static void MerkleComputationOptimized(const std::vector<uint256>& leaves, uint256* proot,
                                     bool* pmutated, uint32_t branchpos, std::vector<uint256>* pbranch) {
    if (pbranch) {
        pbranch->clear();
        // Pre-allocate space for branch to avoid reallocations
        if (leaves.size() > 0) {
            pbranch->reserve(32); // Maximum possible depth
        }
    }
    ...
    while (count < leaves.size()) {
        ...
        // Combine with inner nodes at each level
        for (level = 0; !(count & (1u << level)); level++) {
            ...
            // Check for mutation (duplicate hashes)
    ...
    // Handle the final climb to root
    int level = 0;
...
static void MerkleComputation(const std::vector<uint256>& leaves, uint256* proot, bool* pmutated, uint32_t branchpos, std::vector<uint256>* pbranch) {
    // Keep original implementation exactly as-is for compatibility
...
// Backward compatible functions (unchanged behavior)
...
uint256 BlockMerkleRoot(const CBlock& block, bool* mutated) {
    ...
    leaves.reserve(block.vtx.size()); // Pre-allocate
...
uint256 ComputeMerkleRootOptimized(const std::vector<uint256>& leaves, bool* mutated) {
    uint256 hash;
    MerkleComputationOptimized(leaves, &hash, mutated, -1, nullptr);
    return hash;
}

uint256 ComputeMerkleRootFromTransactions(const std::vector<CTransactionRef>& vtx, bool* mutated) {
    if (vtx.empty()) {
